...
def user_input(user_question, model_name, api_key, pdf_docs, conversation_history):
    ...
    st.snow()
def highlight_and_annotate_pdf(pdf_file, highlights):
    # highlights: list of dicts {page_num, text, answer}
    ...
    for item in highlights:
        ...
        for area in areas:
            ...
            # Add annotation (popup note)
            page.add_text_annot(area.x0, area.y0, answer)
    # Save to bytes
    pdf_bytes = doc.write()
    doc.close()
    return pdf_bytes
...
def main():
    ...
    # PDF Highlighting & Annotation UI
    st.header("PDF Highlighting & Annotated Export")
    if pdf_docs:
        if 'highlights' not in st.session_state:
            st.session_state.highlights = []
        selected_pdf = st.selectbox("Select PDF to annotate", options=pdf_docs, format_func=lambda x: x.name)
        page_num = st.number_input("Page number (0-based)", min_value=0, value=0)
        text_to_highlight = st.text_input("Text to highlight (exact match)")
        answer_annotation = st.text_area("Answer/Annotation for highlight")
        if st.button("Add Highlight & Annotation"):
            ...
            st.success(f"Highlight added for {selected_pdf.name} page {page_num}")
        # Show current highlights
        st.write("Current highlights:")
        for h in st.session_state.highlights:
            st.write(h)
        # Export annotated PDF
        if st.button("Export Annotated PDF"):
            # Only process highlights for selected PDF
            highlights = [h for h in st.session_state.highlights if h['pdf_name'] == selected_pdf.name]
            annotated_pdf_bytes = highlight_and_annotate_pdf(selected_pdf, highlights)
            b64_pdf = base64.b64encode(annotated_pdf_bytes).decode()
            href = f'<a href="data:application/pdf;base64,{b64_pdf}" download="annotated_{selected_pdf.name}"><button>Download Annotated PDF</button></a>'
